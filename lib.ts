...
// Best-effort single-file staging + commit. The tools know exactly which file
// they wrote, so stage only that path: `git add -A` walks the whole memory
// tree (slow on networked mounts) and is the source of index.lock races.
// Failures are silently skipped — the memory files themselves are the source
// of truth; git history is tracking only.
export function gitCommit(config: MemoryConfig, message: string, filePath: string): void {
	...
	try {
		...
		// git not available, not a repo, or transient failure — skip
	}
}
